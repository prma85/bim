...
class JFormFieldForeignKey extends JFormField {
    ...
    protected function getInput() {
        ...
        $select = array();
        $select[] = $db->quoteName($this->key_field);
        if (strpos($this->value_field, 'ONCAT(')) {
            $select[] = $this->value_field;
        } else {
            $select[] = $db->quoteName($this->value_field);
        }
        ...
        switch ($this->input_type) {
            ...
            default:
                ...
                foreach ($results as $result) {
                    if (strpos($this->value_field, 'ONCAT(')) {
                        $options[] = JHtml::_('select.option', $result->{$this->key_field}, $result->val);
                    } else {
