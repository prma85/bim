 ...
 * @since  1.6
 */
class BimModelGoals extends JModelList {
    ...
    protected function getListQuery() {
        ...
        $query->select('`kpi`.title AS kpi, `kpi`.id AS kpi_id');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.`year` LIKE ' . $search . ' OR `kpi`.title LIKE ' . $search . ' )');
            }
        }
    ...
    public function getItems() {
        ...
        foreach ($items as $oneItem) {
            ...
            if (isset($oneItem->kpi_id)) {
                $values = explode(',', $oneItem->kpi_id);
