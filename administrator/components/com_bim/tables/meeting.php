 ...
 * @since  1.6
 */
class BimTableMeeting extends JTable {
    ...
    public function __construct(&$db) {
        JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'BimTableMeeting', array('typeAlias' => 'com_bim.meeting'));
        parent::__construct('#__bim_meeting', 'id', $db);
    }
    ...
    public function bind($array, $ignore = '') {
        ...
        // Support for multiple or not foreign key field: participants
        if (!empty($array['participants'])) {
            if (is_array($array['participants'])) {
                $array['participants'] = implode(',', $array['participants']);
            } else if (strrpos($array['participants'], ',') != false) {
                $array['participants'] = explode(',', $array['participants']);
            }
        } else {
            $array['participants'] = '';
        }
        ...
        if (!JFactory::getUser()->authorise('core.admin', 'com_bim.meeting.' . $array['id'])) {
            $actions = JAccess::getActionsFromFile(
                            JPATH_ADMINISTRATOR . '/components/com_bim/access.xml', "/access/section[@name='meeting']/"
            );
            $default_actions = JAccess::getAssetRules('com_bim.meeting.' . $array['id'])->getData();
            $array_jaccess = array();
    ...
    protected function _getAssetName() {
        ...
        return 'com_bim.meeting.' . (int) $this->$k;
    }
